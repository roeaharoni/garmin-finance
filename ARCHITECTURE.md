...
A Garmin Connect IQ watch app that displays real-time financial quotes (stocks, currencies, crypto) on compatible Garmin watches. The app supports multiple data providers and displays prices with change indicators.
...
├── FinanceTrackerView.mc       # Main view: displays symbols, orchestrates fetching
├── FinanceTrackerDelegate.mc   # Input handler: tap/select to open detail view
├── FinanceWidget.mc            # Glance widget (watch face widget view)
├── DetailView.mc               # Detail view for a single symbol
├── DetailDelegate.mc           # Input handler for detail view (back button)
├── data/
│   ├── models/
│   │   ├── Symbol.mc           # FinanceSymbol class (name, value, change, error state)
...
1. **App Start** → `FinanceTrackerApp.getInitialView()` creates `FinanceTrackerView` + `FinanceTrackerDelegate`
...
5. **Fetch Quotes** → Sequential fetch: one symbol at a time via the selected provider
   ...
   - On response: provider parses API-specific JSON → returns normalized `{ "value", "change", "changePercent" }` dict
   - `DataFetcher` caches result and invokes the view callback with the data dict (or `null` on failure)
   - View updates the `FinanceSymbol` and triggers `fetchNextQuote()` for the next symbol
6. **Display** → `onUpdate()` renders up to 4 symbols with price and change indicator (green +/red -)
7. **Error Handling** → Failed fetches show "Error" in red; symbols awaiting data show "Fetching..." in gray
...
| Method | Description |
|--------|-------------|
| `fetchQuote(symbol, callback)` | Fetch current price for a symbol. Invokes `callback` with a data `Dictionary` or `null` on failure. |
| `fetchHistory(symbol, callback)` | Fetch historical data. Currently stubbed in all providers. |
...
All providers return data in the same dictionary format via the callback:

```monkey-c
{
    "value"         => Lang.Float,    // Current price
...
On failure, the callback receives `null`.
...
**Important patterns:**
- Store the callback as an instance variable (`_callback`) — Monkey C does not support `Method.bindWith()`
- The `onQuoteReceived` method must be **public** (referenced via `method(:onQuoteReceived)`)
...
| Provider | Alias | API Key | Free Tier | Symbol Format | Notes |
|----------|-------|---------|-----------|---------------|-------|
| **Twelve Data** | `twelvedata` | Required | 800 calls/day, 8/min | `USD/EUR`, `BTC/USD`, `SPY` | Default provider. Best free tier. |
| **Alpha Vantage** | `alphavantage` | Required | 25 calls/day | `USDEUR`, `BTC`, `SPY` | Very limited free tier. |
| **Yahoo Finance** | `yahoo` | Not required | Unlimited (unofficial) | `USDEUR=X`, `BTC-USD`, `SPY` | No API key needed, but may be unreliable. |
...
- Manages one pending request at a time (symbol + callback)
- Caches fetched data in `DataCache` (persistent `Application.Storage`)
- Invokes the view's callback with the normalized data dict or `null`
